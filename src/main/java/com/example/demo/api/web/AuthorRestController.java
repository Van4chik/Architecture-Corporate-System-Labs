...
public class AuthorRestController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "OK", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = AuthorsXml.class)),
    ...
    public ResponseEntity<?> list(
        ...
        if (wantsXml(format, request)) return ResponseEntity.ok().contentType(MediaType.APPLICATION_XML).body(xml.toXmlWithXslt(payload, "/xsl/authors.xsl"));
    ...
    public ResponseEntity<?> create(
        ...
        if (wantsXml(format, request)) return ResponseEntity.created(location).contentType(MediaType.APPLICATION_XML).body(xml.toXmlWithXslt(dto, "/xsl/author.xsl"));
